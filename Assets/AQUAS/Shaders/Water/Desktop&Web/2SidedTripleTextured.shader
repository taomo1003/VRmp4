    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i, float facing : VFACE) : COLOR {
                ...
                float2 _multiplier2 = ((i.uv0+(float3((_SmallWavesSpeed/_division1),0.0)*(_timer1.r/100.0)))*_division1);
                float3 _SmallWavesTexture_var = UnpackNormal(tex2D(_SmallWavesTexture,TRANSFORM_TEX(_multiplier2, _SmallWavesTexture)));
                float2 _division2 = ((objScale.rb*_MediumWavesTiling)/_value1);
                float4 _timer2 = _Time + _TimeEditor;
                float2 _multiplier3 = ((i.uv0+(float3((_MediumWavesSpeed/_division2),0.0)*(_timer2.r/100.0)))*_division2);
                float3 _MediumWavesTexture_var = UnpackNormal(tex2D(_MediumWavesTexture,TRANSFORM_TEX(_multiplier3, _MediumWavesTexture)));
                float2 _division3 = ((objScale.rb*_LargeWavesTiling)/_value1);
                float4 _timer3 = _Time + _TimeEditor;
                float2 _multiplier4 = ((i.uv0+(float3(0.0,(_LargeWavesSpeed/_division3))*(_timer3.r/100.0)))*_division3);
                float3 _LargeWavesTexture_var = UnpackNormal(tex2D(_LargeWavesTexture,TRANSFORM_TEX(_multiplier4, _LargeWavesTexture)));
                float3 _add1 = (lerp(float3(0,0,1),_SmallWavesTexture_var.rgb,_SmallWaveRrefraction)+lerp(float3(0,0,1),_MediumWavesTexture_var.rgb,_MediumWaveRefraction)+lerp(float3(0,0,1),_LargeWavesTexture_var.rgb,(pow(saturate((sceneZ-partZ)/_WaveBlend),_WaveFade)*_LargeWaveRefraction)));
