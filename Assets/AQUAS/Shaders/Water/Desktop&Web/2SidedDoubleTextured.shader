    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i, float facing : VFACE) : COLOR {
                ...
                float2 _multiplier6 = ((_rotator2+_multiplier5)*_division2);
                float4 _texture3 = tex2D(_SmallWavesTexture,TRANSFORM_TEX(_multiplier6, _SmallWavesTexture));
                float2 _multiplier7 = ((i.uv0+_multiplier5)*_division2);
                ...
                float2 sceneUVs = float2(1,grabSign)*i.screenPos.xy*0.5+0.5 + ((_subtractor2.rg*(_Refraction*0.2))*_multiplier1);
...
/////// Vectors:
                ...
                float3 _texture2 = ((float3(i.uv0,0.0)+_multiplier4)*float3(_division1,0.0));
                ...
                float3 _texture1 = ((float3(_rotator1,0.0)+_multiplier4)*float3(_division1,0.0));
                float4 _multiplier3 = tex2D(_LargeWavesTexture,TRANSFORM_TEX(_texture1, _LargeWavesTexture));
                float3 _subtractor1 = (_multiplier2.rgb-_multiplier3.rgb);
                float3 _lerp1 = lerp(lerp(_subtractor2,(_subtractor2+_subtractor1),_OffsetSmallBigSmall),_subtractor1,_OffsetSmallBigSmallBig);
